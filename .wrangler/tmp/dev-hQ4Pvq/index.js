...
// .wrangler/tmp/bundle-gUscAA/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-gUscAA/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-gUscAA/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var MyMCP = class extends McpAgent {
  ...
  getApiKey(providedKey) {
    ...
    console.log("globalEnv.RUNWAYML_API_KEY:", globalEnv?.RUNWAYML_API_KEY);
    console.log("Available env keys:", globalEnv ? Object.keys(globalEnv) : "no globalEnv");
    if (globalEnv && globalEnv.RUNWAYML_API_KEY) {
      console.log("Found API key in environment");
      return globalEnv.RUNWAYML_API_KEY;
    }
  ...
  async init() {
    ...
    this.server.tool(
      "runway_text_to_video",
      {
        ...
        api_key: external_exports.string().optional().describe("RunwayML API key (optional - will use RUNWAYML_API_KEY environment variable if not provided)")
      },
      async ({ prompt, model, duration, ratio, api_key }) => {
        const actualApiKey = this.getApiKey(api_key);
        if (!actualApiKey) {
          return {
            content: [
              {
                type: "text",
                text: 'Error: RunwayML API key is required. Please either:\n1. Pass it directly: {"api_key": "your_key_here"}\n2. Set it as a Cloudflare Workers secret named RUNWAYML_API_KEY\n\nGet your API key from: https://dev.runwayml.com'
              }
            ]
          };
        }
...
Use runway_get_task to check the status and get the result.`
    ...
    this.server.tool(
      ...
      async ({ prompt_image, prompt_text, model, duration, ratio, api_key }) => {
        const actualApiKey = this.getApiKey(api_key);
        if (!actualApiKey) {
          return {
            content: [
...
Use runway_get_task to check the status and get the result.`
    ...
    this.server.tool(
      ...
      async ({ prompt_text, model, ratio, reference_images, api_key }) => {
        const actualApiKey = this.getApiKey(api_key);
        if (!actualApiKey) {
          return {
            content: [
